...
def show_lesson5_nasal_finals(add_tones, save_progress, save_score_row_b5, load_all_scores_b5):
    ...
    with tab_diff:
        ...
        pairs = [
            {"label": "Cặp an / ang", "p1": "an", "example1": "fàn (饭 - cơm)", "p2": "ang", "example2": "máng (忙 - bận)"},
            {"label": "Cặp en / eng", "p1": "en", "example1": "hěn (很 - rất)", "p2": "eng", "example2": "péng (朋 - bạn bè)"},
            {"label": "Cặp in / ing", "p1": "in", "example1": "nín (您 - ngài)", "p2": "ing", "example2": "tīng (听 - nghe)"}
        ]
        ...
        for idx, pair in enumerate(pairs):
            ...
            with col_l:
                ...
                render_play_button(pair['p1'], f"🔊 Nghe âm /{pair['p1']}/", key=f"pair_v_{pair['p1']}")
                render_play_button(pair['example1'].split(' (')[0], f"🔊 Từ ví dụ: {pair['example1']}", key=f"pair_ex_{pair['p1']}")
            with col_r:
                st.success(f"👉 **Âm mũi sau (-ng)**: /{pair['p2']}/")
                render_play_button(pair['p2'], f"🔊 Nghe âm /{pair['p2']}/", key=f"pair_v_{pair['p2']}")
                render_play_button(pair['example2'].split(' (')[0], f"🔊 Từ ví dụ: {pair['example2']}", key=f"pair_ex_{pair['p2']}")
            st.markdown("<br/>", unsafe_allow_html=True)
